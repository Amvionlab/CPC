...
function Transfer() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Construct the URL based on user's area
      let url = `${baseURL}/backend/fetchTransfer.php?action=management&status=1`;
      if (user.area === '2') {
  ...
  const handleApprove = async (id) => {
    ...
    try {
      const response = await fetch(`${baseURL}/backend/fetchTransfer.php?action=manageapprove&status=1`, {
        method: 'POST',
  ...
  const handleApproveSelected = async () => {
    ...
    try {
      const response = await fetch(`${baseURL}/backend/fetchTransfer.php?action=manageapprovesel&status=1`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
