...
function Transfer() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      let url = `${baseURL}/backend/fetchTransfer.php?action=recmanagement&status=4`;
  
      // Modify the URL based on the user's area
      if (user.area === '2') {
      ...
      // Make the fetch call with the complete URL
  ...
  const handleApprove = async (id) => {
    ...
    try {
      const response = await fetch(`${baseURL}/backend/fetchTransfer.php?action=manageapprove&status=5`, {
        method: 'POST',
  ...
  const handleApproveSelected = async () => {
    ...
    try {
      const response = await fetch(`${baseURL}/backend/fetchTransfer.php?action=manageapprovesel&status=5`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
