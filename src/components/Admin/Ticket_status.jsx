...
const Form = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${baseURL}/backend/fetchTicket_status.php`);
      ...
      <div className="max-w-5xl m-2 bg-box p-3 rounded-lg font-mont">
       <div className="flex justify-end flex-wrap space-x-2 mt-4">
        ...
        <div className="ticket-table mt-8">
          <h2 className="text-2xl font-bold text-prime mb-4"><span>Ticket Status Data </span><span className="items-end"><button
  ...
  <tbody>
    {currentTickets.map((user) => (
      <tr key={user.id} className="hover:bg-gray-100">
        ...
        <td className="border-t py-4 px-4">{user.status}</td>
