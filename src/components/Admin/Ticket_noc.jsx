...
const Form = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("Ticket NOC added");
      location.reload();
    } catch (error) {
      setSubmissionStatus({
        success: false,
  ...
  const exportCSV = () => {
    ...
    link.setAttribute("download", "Ticket_noc.csv");
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 ml-10 pr-10 mb-0">
                <div className="flex items-center mb-2 mr-4">
                  ...
                  <input
                    type="text"
                    name="name"
                    placeholder="Enter Ticket NOC Name"
                    value={formData.name}
      ...
      <div className="max-w-5xl m-2 bg-box p-3 rounded-lg font-mont">
       <div className="flex justify-end flex-wrap space-x-2 mt-4">
        ...
        <div className="ticket-table mt-8">
          <h2 className="text-2xl font-bold text-prime mb-4"><span>Ticket NOC Data </span><span className="items-end"><button
          onClick={() => setShowForm(!showForm)}
          className="bg-prime font-mont font-semibold text-sm text-white py-2 px-8 rounded-md shadow-md focus:outline-none"
        >
          {showForm ? "Close" : "+ Add Ticket NOC"}
        </button></span></h2>
