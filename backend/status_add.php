...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $checkSql = "SELECT id FROM asset_status WHERE name = ?";
    ...
    if ($stmt->num_rows > 0) {
        ...
        $response = array('success' => false, 'message' => 'Status already exists.');
        echo json_encode($response);
    
    } else {
        // Insert the new location into the 'location' table
        $sql = "INSERT INTO asset_status (name, is_active) VALUES (?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ss", $locationname, $active);

        if ($stmt->execute()) {
            $response = array('success' => true, 'message' => 'Status added successfully.');
            echo json_encode($response);
        } else {
